...
static const tzPair tzTranslations[425] = {
  {"Africa/Abidjan", "GMT0"},
  {"Africa/Accra", "GMT0"},
...
const char* translate(const char* name) {
  for (unsigned int i = 0; i < sizeof(tzTranslations)/sizeof(tzPair); ++i) {
    if (strcmp(tzTranslations[i].name, name) == 0) {
      return tzTranslations[i].posixStr;
    }
  }
  return nullptr;
}